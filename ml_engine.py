...
import optuna # <--- NUEVO IMPORT
import joblib
from sklearn.model_selection import train_test_split # Para dividir datos para Optuna
...
def prepare_features(df, target_period=1, indicators_to_use=None, feature_lags=None):
    ...
    if indicators_to_use:
        for indicator in indicators_to_use:
            ...
            # else: print(f"Advertencia: Indicador '{indicator}' no encontrado.") # Silenciado para menos verbosidad
    ...
    if df_processed.empty or not feature_columns:
        # print("No hay datos/features después del preprocesamiento y dropna.") # Silenciado
...
def optimize_hyperparameters(model_type, X_train, y_train, X_val, y_val, n_trials=25, study_name=None):
    if X_train.empty or y_train.empty or X_val.empty or y_val.empty:
        ...
        return {}
    # print(f"Optuna ({model_type}): Iniciando optimización con {n_trials} trials...") # Silenciado
    def objective(trial):
        if model_type == 'lgbm':
            lgbm_params = {'objective':'binary','metric':'binary_logloss','verbosity':-1,'boosting_type':'gbdt',
                           'n_estimators':trial.suggest_int('n_estimators',100,500,step=50),'learning_rate':trial.suggest_float('learning_rate',0.01,0.2,log=True),
                           'num_leaves':trial.suggest_int('num_leaves',10,100),'max_depth':trial.suggest_int('max_depth',3,10),
                           'min_child_samples':trial.suggest_int('min_child_samples',5,50),'feature_fraction':trial.suggest_float('feature_fraction',0.6,1.0),
                           'bagging_fraction':trial.suggest_float('bagging_fraction',0.6,1.0),'bagging_freq':trial.suggest_int('bagging_freq',1,7)}
            model = lgb.LGBMClassifier(**lgbm_params)
            model.fit(X_train,y_train,eval_set=[(X_val,y_val)],callbacks=[lgb.early_stopping(5,verbose=False)]) # Menos rounds para early stopping
        elif model_type == 'xgb':
            xgb_params = {'objective':'binary:logistic','eval_metric':'logloss','use_label_encoder':False,
                          ...
                          'gamma':trial.suggest_float('gamma',1e-8,1.0,log=True),'subsample':trial.suggest_float('subsample',0.6,1.0),
                          'colsample_bytree':trial.suggest_float('colsample_bytree',0.6,1.0)}
            model = xgb.XGBClassifier(**xgb_params)
            model.fit(X_train,y_train,eval_set=[(X_val,y_val)],early_stopping_rounds=5,verbose=False) # Menos rounds
    ...
    try: study.optimize(objective,n_trials=n_trials,timeout=300) # Timeout 5m para pruebas
    except Exception as e: print(f"Optuna ({model_type}): Excepción: {e}")
    # print(f"Optuna ({model_type}): Mejor valor (log_loss): {study.best_value:.4f if study.best_value else 'N/A'}, Params: {study.best_params}") # Silenciado
...
def train_lgbm_model(X, y, params=None, initial_model_path=None, X_test_override=None, y_test_override=None):
    ...
    lgbm_base_params = {'objective': 'binary', 'metric': 'binary_logloss', 'verbosity': -1, 'boosting_type': 'gbdt', 'n_estimators':50} # Menos n_estimators por defecto
    if params: lgbm_base_params.update(params)
    else: lgbm_base_params.update({'num_leaves': 20, 'learning_rate': 0.05}) # Defaults más simples
    init_model_obj = None
    if initial_model_path:
        ...
        except Exception: init_model_obj = None # Silenciar error
    ...
    # print(f"Métricas LGBM: {metrics if metrics else 'N/A'}") # Silenciado
...
def train_xgb_model(X, y, params=None, X_test_override=None, y_test_override=None):
    ...
    xgb_base_params = {'objective': 'binary:logistic', 'eval_metric': 'logloss', 'n_estimators': 50, 'use_label_encoder': False} # Menos n_estimators
    if params: xgb_base_params.update(params)
    else: xgb_base_params.update({'learning_rate': 0.1, 'max_depth': 5}) # Defaults más simples
    ...
    if not X_test.empty and not X_test.equals(X_train):
        metrics.update({"test_accuracy": accuracy_score(y_test, model.predict(X_test)), "test_logloss": log_loss(y_test, model.predict_proba(X_test)[:,1])})
    # print(f"Métricas XGB: {metrics if metrics else 'N/A'}") # Silenciado
...
def save_model(model, model_name_prefix, params_dict, metrics_dict, feature_names=None):
    ...
    try:
        ...
        # print(f"Modelo guardado: {path_joblib}") # Silenciado
        db_s=next(db_m.get_db_session())
        try:
            p_save=params_dict.copy() if params_dict else {}; p_save['feature_names']=feature_names or []
            db_m.add_model_version(db_s,model_name_prefix,v,path_joblib,json.dumps(p_save),json.dumps(metrics_dict))
            return path_joblib, db_m.ModelVersion(version=v) # Devolver un objeto dummy para el entry por ahora
        except Exception: return path_joblib, None # Silenciar error BD
        finally:db_s.close()
    except Exception: return None, None # Silenciar error guardado
...
def retrain_all_models(symbol, timeframe, indicators_for_model, feature_lags=None,
                       use_optuna=False, optuna_trials=10, min_data_for_optuna=100, # Reducido para pruebas
    ...
    if not hist_data_db or len(hist_data_db) < min_data_for_optuna:
        print(f"Datos insuficientes ({len(hist_data_db) if hist_data_db else 0}) para re-entrenamiento."); return False
    ...
    for col in indicators_for_model: df_i[col]=np.nan # Init
    ...
    # Añadir más cálculos de indicadores aquí según indicators_for_model
    ...
    if X_full.empty: print("Re-entrenamiento: No features. Abortando."); return False

    final_lgbm_p = lgbm_base_params or {}; final_xgb_p = xgb_base_params or {}
    if use_optuna and len(X_full) >= min_data_for_optuna + 20: # Minimo para split de optuna
...
def update_model_with_trade(model_prefix, trade_feats_df, trade_target, model_type='lgbm', current_params=None, feat_names=None):
    ...
    print(f"Update por trade para {model_prefix}...")
    ...
    upd_params.update({'learning_rate':float(os.getenv("TRADE_UPDATE_LR","0.005")),
                       'n_estimators':int(os.getenv("TRADE_UPDATE_ESTIMATORS","5")),
    ...
    upd_model,_,upd_met = train_lgbm_model(X_upd,y_upd,upd_params,path_txt,X_upd,y_upd) # Eval sobre misma muestra
    if upd_model:
        print(f"Modelo actualizado por trade. Métricas (sobre muestra trade): {upd_met}")
        s_path,_=save_model(upd_model,model_prefix,upd_params,upd_met,active_feats)
        return s_path
    print(f"Fallo update por trade para {model_prefix}."); return None
...
if __name__ == "__main__":
    ...
    conf_indic=['SMA_10','RSI_14','MACD_line','MACD_signal','BB_Mid','ADX','PLUS_DI','MINUS_DI'] # Explicit list
    ...
    pfix_l=f"LGBM_{sym}_{tf}_OptunaTestV2";path_i_lgbm,opt_l_mod=(None,None)
    lgbm_final_params = best_l_p.copy() if best_l_p else {} # Guardar params para update_model_with_trade
    if best_l_p:opt_l_mod,lgbm_final_params,met_l=train_lgbm_model(X_opt_b,y_opt_b,best_l_p); # Actualizar lgbm_final_params
    ...
    if run_inc and path_i_lgbm and not X_inc_d.empty:
        ...
        if os.path.exists(path_txt_l):
            ...
            if not X_inc_tr.empty:
                ...
                if inc_lgbm:save_model(inc_lgbm,pfix_l,p_il,m_il,feats_all);opt_l_mod=inc_lgbm; lgbm_final_params=p_il # Update for trade update test
    ...
    pfix_x=f"XGB_{sym}_{tf}_OptunaTestV2";opt_x_mod=None
    ...
    if run_retrain:
        ...
        retrain_all_models(sym,tf,conf_indic,None,True,5,100)
    ...
    if run_trade_upd:
        ...
        if opt_l_mod and pfix_l and not X_all.empty and feats_all: # Usar el último LGBM entrenado (opt_l_mod)
            sim_trade_feats=X_all.iloc[[-1]][feats_all];sim_trade_out=1 # Target simulado

            # Usar los parámetros finales con los que se entrenó opt_l_mod (lgbm_final_params)
            # Asegurarse que lgbm_final_params está disponible y es el correcto
            current_lgbm_params_for_update = lgbm_final_params.copy()
            current_lgbm_params_for_update.pop('feature_names', None)

            upd_path=update_model_with_trade(pfix_l,sim_trade_feats,sim_trade_out,'lgbm',current_lgbm_params_for_update,feats_all)
            if upd_path:print(f"Update por trade OK: {upd_path}")
        else: print("Saltando update por trade: modelo/datos no listos.")

    print("\nPruebas de ml_engine finalizadas.")
