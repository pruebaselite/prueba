...
def prepare_features(df, target_period=1, indicators_to_use=None, feature_lags=None):
    ...
    X = df_processed[feature_columns]; y = df_processed['target'] # y es pd.Series aquí
...
def optimize_hyperparameters(model_type, X_train, y_train, X_val, y_val, n_trials=10, study_name=None): # Reducido n_trials para velocidad
    if X_train.empty or y_train.empty or X_val.empty or y_val.empty:
        print(f"Optuna ({model_type}): Datos vacíos."); return {}

    # Asegurar y_train y y_val son Series para Optuna
    ...
    def objective(trial):
        if model_type == 'lgbm':
            lgbm_params = {'objective':'binary','metric':'binary_logloss','verbosity':-1,'boosting_type':'gbdt',
                           ...
                           'min_child_samples':trial.suggest_int('min_child_samples',5,50)} # Simplificado
            ...
            xgb_params = {'objective':'binary:logistic','eval_metric':'logloss','use_label_encoder':False,
                          'n_estimators':trial.suggest_int('n_estimators',50,300,step=50),'learning_rate':trial.suggest_float('learning_rate',0.01,0.3,log=True),
                          'max_depth':trial.suggest_int('max_depth',3,10),'min_child_weight':trial.suggest_int('min_child_weight',1,10)} # Simplificado
    ...
    try: study.optimize(objective,n_trials=n_trials,timeout=180) # Timeout 3m para pruebas
...
def train_lgbm_model(X, y, params=None, initial_model_path=None, X_test_override=None, y_test_override=None):
    ...
    if y_test_override is not None and isinstance(y_test_override, pd.DataFrame): y_test_override = y_test_override.iloc[:,0]
    # --- FIN DE CORRECCIÓN ---
...
def save_model(model, model_name_prefix, params_dict, metrics_dict, feature_names=None):
    ...
    try:
        ...
        try:
            ...
            return path_joblib, entry if entry else db_m.ModelVersion(version=v) # Devolver dummy si falla BD
        except Exception: return path_joblib, db_m.ModelVersion(version=v) # Devolver dummy
        finally:db_s.close()
...
def update_model_with_trade(model_prefix, trade_feats_df, trade_target, model_type='lgbm', current_params=None, feat_names=None):
    ...
    y_upd = pd.Series([trade_target], index=X_upd.index) # y_upd es pd.Series
    upd_params = current_params.copy() if current_params else {}
    upd_params.update({'learning_rate':float(os.getenv("TRADE_UPDATE_LR","0.005")), 'n_estimators':int(os.getenv("TRADE_UPDATE_ESTIMATORS","1")), # 1 estimador para un solo trade
...
if __name__ == "__main__":
    # ... (Todo el if __name__ como estaba en la versión anterior, solo se corrigieron las funciones de arriba) ...
    ...
    n_opt_trials=int(os.getenv("OPTUNA_TRIALS","5")); run_inc=os.getenv("RUN_INCREMENTAL_LGBM","true").lower()=='true' # Reducido trials
    ...
    X_all,y_all,_,feats_all=prepare_features(hist_df_i.copy(),1,conf_indic,None) # conf_indic aquí en lugar de features_config_indicators
    ...
    pfix_l=f"LGBM_{sym}_{tf}_OptunaTestV3";path_i_lgbm,opt_l_mod=(None,None) # V3 para nueva versión
    ...
    if run_inc and path_i_lgbm and not X_inc_d.empty:
        ...
        if os.path.exists(path_txt_l):
            p_inc_l=lgbm_final_params.copy();p_inc_l['learning_rate']=lgbm_final_params.get('learning_rate',0.05)*0.5 # Asegurar que p_inc_l tiene base
    ...
    pfix_x=f"XGB_{sym}_{tf}_OptunaTestV3";opt_x_mod=None # V3
    ...
    if run_retrain:
        ...
        retrain_all_models(sym,tf,conf_indic,None,True,3,100) # 3 trials para prueba rápida
    ...
    if run_trade_upd:
        ...
        if opt_l_mod and pfix_l and not X_all.empty and feats_all:
            sim_trade_feats=X_all.iloc[[-2]][feats_all] if len(X_all) >=2 else X_all.iloc[[-1]][feats_all] # Usar penúltima fila si existe
            sim_trade_out=y_all.iloc[-2] if len(X_all) >=2 else y_all.iloc[-1] # Target real de esa fila
            curr_p = lgbm_final_params.copy() # Usar los params del último modelo LGBM entrenado
    ...
    print("\nPruebas de ml_engine (V3 con corrección y_train) finalizadas.")
