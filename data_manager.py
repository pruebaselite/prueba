...
import database_manager as db_m # db_m en lugar de database_manager para brevedad
...
def calculate_rsi(data_series, window=14):
    ...
    # CORRECCIÓN: Usar ffill() directamente
...
def calculate_adx(df_ohlc, window=14):
    ...
    if not all(col in df_ohlc.columns for col in ['high', 'low', 'close']):
        raise ValueError("El DataFrame debe contener columnas 'high', 'low', 'close'.")
    # ADX necesita más datos, al menos window periodos para ATR y luego window para DM y window para ADX suavizado.
    # Un aproximado seguro es window * 3, pero incluso más es mejor para la calidad del ADX.
    # Si len(df_ohlc) < window * 2 es muy poco.
    if len(df_ohlc) < window + window + window -1 : # Mínimo para ATR, luego DM, luego ADX
    ...
    plus_di_series = (smooth_plus_dm / df[f'ATR_{window}'].replace(0, np.nan)) * 100
    minus_di_series = (smooth_minus_dm / df[f'ATR_{window}'].replace(0, np.nan)) * 100

    plus_di_series.fillna(0, inplace=True)
    minus_di_series.fillna(0, inplace=True)

    dx_denominator = (plus_di_series + minus_di_series).replace(0, np.nan)
    dx = (abs(plus_di_series - minus_di_series) / dx_denominator) * 100
    dx.fillna(0, inplace=True) # Si plus_di y minus_di son 0, DX es 0. Si el denominador fue NaN, también.

    adx = dx.ewm(alpha=1/window, adjust=False, min_periods=window-1).mean()

    return plus_di_series.rename(f'PLUS_DI_{window}'), minus_di_series.rename(f'MINUS_DI_{window}'), adx.rename(f'ADX_{window}')
...
try:
    import MetaTrader5 as mt5
    TIMEFRAME_M1, TIMEFRAME_M5, TIMEFRAME_M15, TIMEFRAME_M30 = mt5.TIMEFRAME_M1, mt5.TIMEFRAME_M5, mt5.TIMEFRAME_M15, mt5.TIMEFRAME_M30
    TIMEFRAME_H1, TIMEFRAME_H4, TIMEFRAME_D1, TIMEFRAME_W1, TIMEFRAME_MN1 = mt5.TIMEFRAME_H1, mt5.TIMEFRAME_H4, mt5.TIMEFRAME_D1, mt5.TIMEFRAME_W1, mt5.TIMEFRAME_MN1
    MT5_AVAILABLE = True
except ImportError:
    print("Advertencia: El paquete MetaTrader5 no está instalado. Funcionalidad MT5 no disponible.")
    MT5_AVAILABLE = False
    TIMEFRAME_M1, TIMEFRAME_M5, TIMEFRAME_M15, TIMEFRAME_M30 = 1, 2, 3, 4 # Placeholders
    TIMEFRAME_H1, TIMEFRAME_H4, TIMEFRAME_D1, TIMEFRAME_W1, TIMEFRAME_MN1 = 5, 6, 7, 8, 9 # Placeholders
...
def initialize_mt5_connection(login, password, server, path):
    ...
    if path and path.lower() != 'none' and path.strip() != '':
        init_kwargs['path'] = path

    if not mt5.initialize(**init_kwargs):
        print(f"Error en mt5.initialize() con path '{init_kwargs.get('path', 'N/A')}': {mt5.last_error()}")
        if 'path' in init_kwargs:
...
def shutdown_mt5_connection():
    if MT5_AVAILABLE and mt5.terminal_info(): # Solo apagar si está inicializado
        mt5.shutdown()
...
def get_historical_data_mt5(symbol, timeframe_mt5_const, num_bars=1000, start_date=None, end_date=None):
    if not MT5_AVAILABLE or not mt5.terminal_info():
        ...
        return None
    print(f"Descargando datos de MT5 para {symbol} en timeframe {timeframe_mt5_const}...")
    rates = None
    try:
        if start_date and end_date: rates = mt5.copy_rates_range(symbol, timeframe_mt5_const, start_date, end_date)
        elif start_date: rates = mt5.copy_rates_range(symbol, timeframe_mt5_const, start_date, datetime.now())
        else: rates = mt5.copy_rates_from_pos(symbol, timeframe_mt5_const, 0, num_bars)
    except Exception as e:
        print(f"Excepción al llamar a mt5.copy_rates: {e}. MT5 last_error: {mt5.last_error()}")
        return None
    if rates is None or len(rates) == 0:
        print(f"No se pudieron obtener datos de MT5 para {symbol}: {mt5.last_error()}")
        return None
    ...
    elif 'volume' not in rates_df.columns: rates_df['volume'] = 0
    rates_df = rates_df[['timestamp', 'open', 'high', 'low', 'close', 'volume']]
    print(f"Descargados {len(rates_df)} registros de MT5 para {symbol}.")
    return rates_df
...
def store_historical_data(df, symbol, timeframe_str):
    ...
    try:
        # Para evitar duplicados si se corre múltiples veces, podríamos borrar datos existentes
        # O manejarlo con una clave única en la BD (symbol, timeframe, timestamp)
        # Por ahora, simplemente añade. Considerar una estrategia de actualización/borrado.
        db_m.add_historical_data(session, data_list)
        print(f"Datos para {symbol}/{timeframe_str} almacenados/actualizados en la base de datos.")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    # Eliminar BD para pruebas limpias si es necesario, o manejar duplicados
    # if os.path.exists(db_m.DATABASE_NAME):
    #     print(f"Eliminando BD existente: {db_m.DATABASE_NAME} para prueba limpia.")
    ...
    # Usar un símbolo y timeframe consistentes para las pruebas
    test_symbol = os.getenv("BOT_SYMBOL", "SIM_EURUSD")
    test_timeframe_str = os.getenv("BOT_TIMEFRAME", "H1")

    timeframe_map_str_to_const = { "H1": TIMEFRAME_H1, "M1": TIMEFRAME_M1 } # Expandir si es necesario
    ...
    if mt5_connected and test_timeframe_mt5 is not None:
        ...
        # Descargar suficientes datos para que los indicadores se calculen bien
        num_bars_mt5 = int(os.getenv("MT5_DOWNLOAD_BARS", "300")) # Configurable
        end_dt = datetime.now()
        start_dt = end_dt - timedelta(days=num_bars_mt5 // (24 if test_timeframe_str == "H1" else 1440//int(test_timeframe_str.replace("M","")) ) + 5) # Aproximación

        historical_df = get_historical_data_mt5(test_symbol, test_timeframe_mt5, num_bars=num_bars_mt5, start_date=start_dt, end_date=end_dt)
    ...
    if historical_df is None:
        print("Generando y almacenando datos simulados para pruebas de indicadores...")
        num_sim_points = int(os.getenv("SIM_DATA_POINTS", "300")) # AUMENTADO
        sim_dates = pd.to_datetime([datetime(2023, 1, 1) + timedelta(hours=i) for i in range(num_sim_points)])
        data = {
            'timestamp': sim_dates,
            'open': np.random.rand(num_sim_points) * 10 + 1.0500, # Alrededor de un precio base
        }
        data['high'] = data['open'] + np.random.rand(num_sim_points) * 0.0050 # Movimientos más realistas
        data['low'] = data['open'] - np.random.rand(num_sim_points) * 0.0050
        data['close'] = data['open'] + (np.random.rand(num_sim_points) - 0.5) * 0.0060
        ...
        store_historical_data(historical_df, test_symbol, test_timeframe_str) # Guardar para que otros lo usen
    ...
    if historical_df is not None and not historical_df.empty:
        ...
        print("\n--- Calculando Indicadores (sobre datos simulados o de MT5) ---")
        ...
        macd_line, macd_signal, _ = calculate_macd(df_for_indicators['close'])
        ...
        df_for_indicators['BB_Mid'] = bb_mid; df_for_indicators['BB_Upper'] = bb_upper; df_for_indicators['BB_Lower'] = bb_lower
        _, _, adx_series = calculate_adx(df_for_indicators[['high', 'low', 'close']])
        df_for_indicators['ADX'] = adx_series

        print("\nDataFrame con todos los indicadores (últimas filas donde los cálculos son válidos):")
        cols_to_show = ['open', 'high', 'low', 'close', 'SMA_10', 'RSI_14', 'MACD_line', 'BB_Mid', 'ADX']
        # Mostrar solo filas donde ADX no es NaN, ya que es uno de los que tarda más en calcularse
        valid_indicators_df = df_for_indicators[cols_to_show].dropna()
        print(valid_indicators_df.tail(15))
        print(f"Total de filas con indicadores válidos: {len(valid_indicators_df)}")
    else:
    ...
    print("\nPruebas de data_manager (con corrección de fillna y datos simulados aumentados) finalizadas.")
